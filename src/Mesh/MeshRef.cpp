...
void MeshRef::regenEnvironmentMap(glm::mat4 model, std::function<void(glm::mat4, glm::mat4)> renderCallback) {
	Global::drawToCubemap(environmentMap, getUnmodeledCenter() + glm::vec3(model[3]), fbo, renderCallback);
}
...
void MeshRef::initialize() {
	CHECK_GL_ERROR();
	SPDLOG_DEBUG("Initializing MeshRef environment map...");
	int oldFbo, oldRbo;
	...
	CHECK_GL_ERROR();
	for (int i = 0; i < 6; i++) {
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, ENVMAP_SIZE.x, ENVMAP_SIZE.y, 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr);
		CHECK_GL_ERROR();
	}
